...
**J279: 퀘스트 1.  GPT를 Perplexity처럼! **

## 수행 과정

### 질문 및 답변
- 학습 중 궁금했던 주제: **TCP 3-way handshake 과정**
- **프롬프트** :TCP의 3-way handshake 과정을 단계별로 설명하고, 출처(URL)나 참고 문서와 함께 알려줘.

- **gpt 답변**:
TCP 3-Way Handshake 과정
SYN (Synchronize)
클라이언트가 서버에 연결 요청을 보내며, 이 패킷에는 랜덤 초기 시퀀스 번호 
A가 포함됩니다.
클라이언트는 이 단계에서 SYN 플래그를 설정합니다.
이런 과정을 통해 클라이언트는 연결 의사를 서버에 전달합니다.

SYN‑ACK (Synchronize‑Acknowledge)
서버는 SYN을 받고 연결 요청을 수용한다면 응답으로 SYN‑ACK 패킷을 보냅니다.
이 패킷에는 서버의 초기 시퀀스 번호 B와, 클라이언트의 시퀀스 번호 A+1에 대한 ACK 번호가 포함됩니다.

ACK (Acknowledge)
클라이언트는 서버의 SYN‑ACK을 받은 후, ACK 패킷을 서버로 보냅니다.
이때 시퀀스 번호는 이전에 보낸 ACK 번호 A+1, 그리고 ACK 번호는 서버의 시퀀스 번호 B+1이 됩니다.
이 단계 이후, 클라이언트와 서버는 전이중(full‑duplex) 통신을 위한 연결이 성립됩니다.


**참고 문서 및 공식 출처**
RFC 793 (원본 TCP 표준 문서)
- TCP 연결 수립 절차(3‑Way Handshake) 설명. 
- 또한, 구형 연결 요청의 혼란을 방지하기 위한 설계 목적도 명시되어 있습니다. 

RFC 9293 (최신 TCP 표준 업데이트)
- 여전히 3‑Way Handshake 절차가 TCP 연결에 필수적인 과정임을 강조합니다. 

MDN: https://developer.mozilla.org/en-US/docs/Glossary/TCP_handshake


### 결과

- 신뢰할 수 있는 **공식 표준 문서 + 학습용 보조 자료**를 확보
- AI가 준 정보와 실제 출처 내용을 대조한 결과 **불일치 없음**

## 인사이트

1. **AI 답변의 정확성은 출처 검증이 중요**  
   출처를 확인하기 전까지는 “정답”이라고 단정하기 어렵다.

2. **공식 문서와 학습용 자료의 병행이 효과적**  
   표준 문서는 근거로, 보조 자료는 이해도를 높이는 데 유용했다.

3. **출처를 습관적으로 남기면 재학습 효율이 상승**  
   나중에 동일 주제를 복습할 때 불필요한 재검색 시간을 절약할 수 있다.
